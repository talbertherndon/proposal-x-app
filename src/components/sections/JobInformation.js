...
export default function JobInformation({ setJobInformation, jobInformation }) {
    ...
    const renderItem = ({ item, index }) => {
        return (
            <>
                <ListItem onPress={() => { setEditMode({ ...item, index }); setVisible(true); setMode('edit'); }} accessoryRight={(props) => (<Button onPress={() => { const newItems = [...jobInformation]; newItems.splice(index, 1); setJobInformation(newItems) }} size="tiny">Delete</Button>)}
                    accessoryLeft={(props) => (<Image style={{ width: 75, height: 75, borderRadius: 5 }} source={item.source} />)} title={props => <Text style={{ marginHorizontal: 10, fontSize: 18, fontWeight: 500 }}>{item.name}</Text>} description={des => <View style={{ marginHorizontal: 10 }}><Text category="c1">{item.requirements.map((requires) => requires.name).join(', ')}</Text><Text category="c2">${item.estimate}</Text></View>} />
            </>
        )
    }
    ...
    return (
        <>
            <Layout style={styles.container}>
                ...
                <Button onPress={() => { setVisible(true); setEditMode(); setMode('add'); }} style={{ marginRight: 'auto' }}>Add Areas</Button>

                <Modal visible={visible} backdropStyle={styles.backdrop} onBackdropPress={() => setVisible(false)}>
                    <NewAreaModal mode={mode} setVisible={setVisible} addAreaHandler={addAreaHandler} editAreaHandler={editAreaHandler} editMode={editMode} />
                </Modal>
